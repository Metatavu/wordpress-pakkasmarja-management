<?php
  ...
  if (!class_exists( '\Metatavu\Pakkasmarja\ChatThreads\GroupsWidget' ) ) {
    ...
    class GroupsWidget {
      ...
      public function addMetaBoxes() {
        $metaboxTitle = __('User Groups', 'pakkasmarja_management');
      	...
      	add_meta_box('chat-thread-properties-meta-box', $metaboxTitle, [$this, 'renderChatThreadMetaBox'], 'chat-thread', 'normal', 'default');
      }
      
      public function renderChatThreadMetaBox($chatThread) {
        $userGroupIdOptions = get_post_meta($chatThread->ID, "pm-user-group-options", true);
      ...
      public function saveUserGroups($chatThreadId) {
        ...
        update_post_meta($chatThreadId, 'pm-user-group-options', $userGroupIdOptions);
      }
